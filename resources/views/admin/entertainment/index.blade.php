    ...
    <div class="mainTabs">
        <div class="row" style="border-bottom: 1px solid; padding-block: 10px; margin-bottom: 10px;">
            <div class="col text-start" style="font-family: 'Inter',sans-serif; font-weight: bold; font-size: 1.5em;">UPDATED
                ENTERTAINMENT ARTICLE</div>
            <div class="col text-end">
                <a href="{{ route('entertainment.create') }}" class="btn btn-primary">ADD NEW</a>
            </div>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex gap-2">
                                <a href="{{ route('entertainment.edit', $item->id) }}" class="btn btn-warning">EDIT</a>
                                <a href="{{ route('entertainment.destroy', $item->id) }}" class="btn btn-danger delete-news"
                                    data-id="{{ $item->id }}" onclick="event.preventDefault();">
                                    DELETE
                                </a>

                                <form id="delete-form-{{ $item->id }}"
                                    action="{{ route('entertainment.destroy', $item->id) }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                </form>
